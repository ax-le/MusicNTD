...
class NTDTests(unittest.TestCase):
    ...
    def test_invalid_ranks_values(self):
        with self.assertRaises(err.InvalidRanksException):
            core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,10], init = "random", return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
        # with self.assertRaises(err.InvalidRanksException):
        #     core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,120,40], init = "random", return_errors = True, verbose = False,
        #                             sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
    
    def test_invalid_init_values(self):
        with self.assertRaises(err.InvalidInitializationType):
            core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,10,10], init = "string", return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])

    def test_invalid_custom_init_values(self):
        with self.assertRaises(err.CustomNotEngouhFactors):
            core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,10,10], init = "custom", factors_0 = [self.factors_0, self.factors_1], return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
        
        with self.assertRaises(err.CustomNotValidFactors):
            core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,10,10], init = "custom", factors_0 = [self.factors_0, self.factors_1, None], return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
            
        with self.assertRaises(err.CustomNotValidCore):
            core, facs, errs, toc = ntd(self.init_by_product_tensor, [30,10,10], init = "custom", factors_0 = [self.factors_0,self.factors_1,self.factors_2], core_0 = None, return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
    ...
    def test_good_random_decomp(self):
        core, facs, errs, toc = ntd(self.init_by_product_tensor, self.random_ranks, init = "random", return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
        self.assertAlmostEqual(errs[-1], 0, places = 2)
        self.assertTrue(self.strictly_decreasing(errs))
        
    def test_good_tucker_decomp(self):
        core, facs, errs, toc = ntd(self.init_by_product_tensor, self.random_ranks, init = "tucker", return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
        self.assertAlmostEqual(errs[-1], 0, places = 2)
        self.assertTrue(self.strictly_decreasing(errs))
    ...
    def test_good_custom_decomp(self):
        core, facs, errs, toc = ntd(self.init_by_product_tensor + np.random.random(self.random_shape_tens), self.random_ranks, init = "custom", factors_0 = [self.factors_0, self.factors_1, self.factors_2], core_0 = self.core,
                                    return_errors = True, verbose = False,
                                    sparsity_coefficients = [None, None, None, None], normalize = [False, False, False, False])
        self.assertAlmostEqual(errs[-1], 0, places = 2)
        self.assertTrue(self.strictly_decreasing(errs))
    ...
    def preserve_error_n_iterations(self, tensor, ranks, nb_iter = 2, n_iter_ntd = 100, hals = False, init = "random",
        ...
        first_iteration = ntd(tensor, ranks, init = init, n_iter_max=n_iter_ntd, tol=1e-6,
                    ...
                    verbose=False, return_errors=True, deterministic = deterministic)
