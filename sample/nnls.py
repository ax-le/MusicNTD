...
def hals_nnls_acc(UtM, UtU, in_V, maxiter=500, atime=None, alpha=0.5, delta=0.01,
    ...
    r, n = np.shape(UtM)
    if not in_V.size:
        raise err.ArgumentException("Missing initialization factor.")
    else:
        V = in_V.copy()
        
    if len(UtU.shape) == 1:
        raise err.ArgumentException("Incorrect shape for UtU, you could use reshape(<shape>, 1) to correct this problem if UtU is a vector.")
